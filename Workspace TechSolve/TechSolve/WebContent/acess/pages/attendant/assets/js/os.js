		...
		success: async (data) => {
			...
			for(var i=0; i<dataObj.length;i++){
				...
				if(dataObj[i].status==3){ //ORÇAMENTO EM APROVAÇÃO
					...
					htmlForm+=
							"<div>"
							+"<input type='button' value='Realizar pagamento' onclick='TECHSOLVE.order.doPayment("+ dataObj[i].idordem_servico +")' class='btn btn-warning inpt-btn-box'>"
							+"</div>";
				}
...
TECHSOLVE.order.doPayment = (idOS) => {
	
	$("#modalWarning").html("");
	$("#modalWarning").html("<p>Cliente efetuou o pagamento?</p>");
	
	var modalDoPayment = {

		title: "Confirmar/Reprovar pagamento",
		height: 183,
		width: 535,
		modal: true,
		buttons: {
			"PAGO": function(){
				$.get(TECHSOLVE.RESTPATH + "order/doPayment", "idOS="+idOS, (msg)=>{
					TECHSOLVE.showPopUp("Mensagem", msg, null);
					TECHSOLVE.order.getall();
				});
			},
	...
	$("#modalWarning").dialog(modalDoPayment);
