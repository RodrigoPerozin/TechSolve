...
TECHSOLVE.client.getallForSelect = function(){
	...
	$.ajax({
		...
		success: function(data){
			...
			optionsHTML += "<option value=''>Selecione aqui</option>"
			for(var i=0;i<dataObj.length;i++){	
				cpf_formated = dataObj[i].cpf.toString().slice(0, 3)+"."+dataObj[i].cpf.toString().slice(3, 6)+"."+dataObj[i].cpf.toString().slice(6, 9)+"-"+dataObj[i].cpf.toString().slice(9, 11);
				optionsHTML += "<option value='"+dataObj[i].idcliente+"'>" + dataObj[i].nome + " - " + cpf_formated + "</option>";
			}
						
			if(document.getElementById("client")==undefined){
				console.log(data)
			}else{
			$("#client").html("");
			$("#client").html(optionsHTML);
			}
...
TECHSOLVE.client.filterClientSelect = () => {
	var filterTxt = $("#filtroCliente").val();
	
	$.ajax({
		type: "GET",
		url: TECHSOLVE.RESTPATH + "client/filterby",
		data: "filterTxt="+filterTxt,
		success: function(data){

			if(data.length!=0){
				var cpf_formated;
				$("#client").html("");
				$("#client").append("<option value>Selecione aqui</option>");
				for(var i=0;i<data.length;i++){
					cpf_formated = data[i].cpf.toString().slice(0, 3)+"."+data[i].cpf.toString().slice(3, 6)+"."+data[i].cpf.toString().slice(6, 9)+"-"+data[i].cpf.toString().slice(9, 11);
					$("#client").append("<option value='"+ data[i].idcliente +"'>" + data[i].nome + " - " + cpf_formated + "</option>");
				}
			}				

		},
		error: function(info){
			TECHSOLVE.showPopUp("Mensagem", "Erro ao filtrar cliente: " + info.responseText + " - " + info.status + " - " + info.statusText, null);
		}
	});
