...
public class OrderRest extends UtilRest{
	...
	@Path("/doPayment")
	@Consumes("application/*")
	public Response doPayment(@QueryParam("idOS") int idOS) {
		...
		try {
			...
			Connection conn = con.openConn();
			JDBCOrderDAO jdbcOrder = new JDBCOrderDAO(conn);
			int respIndex = jdbcOrder.doPayment(idOS);
			
			if(respIndex==1) {
				msg = "Pagamento realizado com sucesso! O sistema deu baixa.";
			}else if(respIndex==2){
				msg = "Erro ao efetuar o pagamento!";
			}
